...
1. 数据劫持---\>观察者---\>数据解析、指令、js对象，diff算法---\>dom。

**那么vue是通过什么进行数据劫持的呢**？

1. vue2 是通过Object.defineProperty。IE8以下不兼容
2. vue3 则是通过proxy。IE9以下都不兼容。Edge 12+
...
1. target 原对象 （obj）
2. p表示设置的值的key，比如我们设置的是a的值，那么这个p表示的key就是a。
3. value表示，新的值。
4. receiver表示，原对象的proxy格式。
...
**1. 基本上 Proxy 针对数据的拦截，包括，get/set/has/deleteProperty
2. 遍历相关的拦截，包括，ownKeys
3. 属性描述器的拦截，包括，getOwnPropertyDecriptor/defineProprerty
4. Reflect.isExtensble(newTarget)属性描述器中configurable的值。**
